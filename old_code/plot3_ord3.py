...
data = np.genfromtxt('res_ord3.txt', names=['x1', 'x2', 'x3', 'x4', 'x5', 'x6'])
...
max_ordinal = 3, min_skill_time = 500, max_skill_time = 6000, time_step = 600s") 
...
ax1.plot(data['x2'][np.where(data['x1']==1)], data['x4'][np.where(data['x1']==1)], label='Algo 1')
ax1.plot(data['x2'][np.where(data['x1']==2)], data['x4'][np.where(data['x1']==2)], label='Algo 2')

#ax1.errorbar(1-x_unique, y_mean, y_error, fmt='-o', label='mean and std')


leg = ax1.legend()
plt.savefig('myplot3_ord3.pdf', format='pdf')
