...
ax1.plot(data['x2'][np.where(data['x1']==1)], data['x5'][np.where(data['x1']==1)], label='Algo 1')
ax1.plot(data['x2'][np.where(data['x1']==2)], data['x5'][np.where(data['x1']==2)], label='Algo 2')

#ax1.errorbar(1-x_unique, y_mean, y_error, fmt='-o', label='mean and std')


leg = ax1.legend()
plt.savefig('myplot1_util.pdf', format='pdf')
